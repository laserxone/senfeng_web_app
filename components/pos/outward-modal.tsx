...
const OutwardModal = ({ visible, onClose }: { visible: boolean, onClose: Dispatch<SetStateAction<boolean>>, }) => {
    ...
    const columns: ColumnDef<OutwardProps>[] = [
        {
            ...
            cell: ({ row }) => <div className="ml-2">{row.getValue("invoicenumber")}</div>,
        },
        {
            accessorKey: "company",
            filterFn: "includesString",
                    ...
                    <div className="flex gap-1">
                        ...
                        >
                            <AccordionItem value="fields" className="border-none">
                                ...
                                <AccordionContent className="space-y-2">
                                    {currentItem?.fields && currentItem?.fields?.map((item, i) => (
                                        <div
                                            key={i}
                        ...
                        {currentItem?.outward_gatepass ?
                            ...
                            <Button variant={"ghost"} size={"icon"} onClick={() => {
                                ...
                                setItems(currentItem?.fields?.map((item) => ({ ...item, name: item?.name || item?.description })))
                            }}>
                                <Edit />
                            </Button>
                        }
    ...
    const handleItemChange = (
        ...
        if (field === "isExisting") {
            const copy = [...items];
            copy[index] = { ...copy[index], [field as string]: value, name: "", qty: 0, remarks: "", unit: "", };
            setItems(copy);
        } else {
            const copy = [...items];
            ...
            <DialogContent className={`transition-all duration-300 ${createOutward ? "sm:max-w-[90vw] w-[90vw]" : "sm:max-w-4xl w-full"
                ...
                <div className="w-full flex flex-1">
                    <ScrollArea className="px-2 w-full h-[85dvh]">
                        ...
                        >
                            <div className={`${createOutward ? "w-2/3" : "w-full"} px-2 space-y-2`}>
                                <PageTable
                                    columns={columns}
                                    data={data}
                                    loading={loading}
