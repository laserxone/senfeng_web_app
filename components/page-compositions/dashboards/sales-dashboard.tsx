  ...
  const RenderCallTab = useCallback(() => {
    return (
      <Card className="flex flex-1 p-0">
        <CardContent className="flex flex-1 p-0 pt-0">
          <Calls
            data={callData}
            onRefresh={async () => {
              const startDate = moment().startOf("month").toISOString()
      ...
      <div className="flex flex-1 flex-col gap-4">
          ...
          <SalesMetricCard
            ...
            onClick={() =>
              setSelectedMetric({
                ...
                totalAmount:
                  data?.new_entries?.pending_parts_payments?.total_amount,
        ...
        <div hidden={activeTab !== "issued"}>
          <RenderReturnable
            userID={userID}
            height="min-h-[calc(100dvh-420px)]"
            onUpdateTotal={(val) => setAllReturnables(val)}
          />
        </div>
        <div hidden={activeTab !== "fines"}>
          <RenderFines
            userID={userID}
            height="min-h-[calc(100dvh-480px)]"
...
function CustomersTab({ data }: { data: SalesCustomer[] }) {
    ...
    return (
      <div className="flex w-full flex-col gap-2 rounded-lg border bg-background px-3 py-2.5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:justify-end">
          <Badge
            variant="outline"
            className="h-6 rounded-full bg-slate-50 px-2 text-[10px] dark:bg-zinc-900"
          >
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-b bg-slate-50/80 bg-white px-4 py-3 shadow-sm dark:bg-zinc-900/70 dark:bg-zinc-950">
        <div className="flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Badge
            variant="outline"
            className="rounded-full bg-background px-2.5 text-[11px]"
          >
          ...
          >
            {localData.reduce((sum, item) => sum + item.sales.length, 0)}{" "}
            machines
          </Badge>
        </div>
      </div>
      ...
      <Accordion
        type="single"
        collapsible
        className="mt-2 w-full space-y-2 px-1"
      >
        {localData.length === 0 ? (
          ...
          localData.map((customer) => (
            ...
            >
              <Card className="overflow-hidden border-0 bg-white p-0 shadow-sm ring-1 ring-slate-200/80 transition-shadow hover:shadow-md dark:bg-zinc-950 dark:ring-white/10">
                <AccordionTrigger className="hover:no-underline sm:px-4">
                  <div className="w-full">
                    ...
                    <Progress
                      value={Number(customer.overall || 0)}
                      className="mt-2"
                    />
                  </div>
                </AccordionTrigger>
...
function Calls({
  ...
  const RenderEachCall = ({ call }: { call: UserCallData }) => {
    return (
      <Card
        key={call.id}
        className="group overflow-hidden border-0 bg-white shadow-sm ring-1 ring-slate-200/80 transition-shadow hover:shadow-md dark:bg-zinc-950 dark:ring-white/10"
      >
    ...
    <div className="w-full min-w-0 overflow-hidden rounded-xl border bg-white shadow-sm dark:bg-zinc-950">
      <div className="flex flex-col gap-3 border-b bg-slate-50/80 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:bg-zinc-900/70">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <h3 className="text-sm leading-none font-semibold">Call Queue</h3>
            <p className="mt-1 text-xs text-muted-foreground">
        ...
        <Badge
          variant="outline"
          className="w-fit rounded-full bg-background px-2.5 text-[11px]"
        >
      ...
      <DialogContent className="w-full max-w-[94vw] overflow-hidden rounded-2xl border-border bg-card p-0 text-card-foreground transition-all duration-300 sm:max-w-xl">
        <DialogHeader className="border-b border-border bg-muted/40 px-4 py-3">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <Badge
              variant="outline"
              className="ml-auto w-fit shrink-0 rounded-full bg-background px-2 py-0.5 text-[10px]"
            >
              {machineData.length}{" "}
              {machineData.length === 1 ? "machine" : "machines"}
            </Badge>
          </div>
        </DialogHeader>
                ...
                >
                  ...
                  <span className="shrink-0 text-right">
                    ...
                    <span className="block text-[10px] text-muted-foreground">
                      {item.contract_date
                        ? moment(item.contract_date).format("DD MMM YYYY")
                        : "-"}
                    </span>
                  </span>
