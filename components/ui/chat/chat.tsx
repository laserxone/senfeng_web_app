...
function ChatVoiceMessage({
  ...
  return (
    <div className="mt-1.5 flex items-center gap-3">
      ...
      <span className="text-[12px] shrink-0 opacity-60 tabular-nums">
        {timeLabel}
      </span>
    </div>
  );
            ...
            {message.code && (
              <div className="chat-content-card mt-1.5 overflow-hidden">
                <div className="flex items-center justify-between bg-[var(--chat-bg-code)] px-3 py-1.5">
                  <span className="text-[11px] font-medium text-[var(--chat-text-tertiary)]">
                    {message.code.language}
                  </span>
                </div>
                <pre className="overflow-x-auto bg-[var(--chat-bg-code)] px-3 py-2">
                  <code
                    className="text-[13px] leading-relaxed text-[var(--chat-text-primary)]"
            ...
            {message.files && message.files.length > 0 && (
              <div className="mt-1.5 flex flex-col gap-1.5">
                {message.files.map((file, idx) => (
                  <div
                    key={idx}
                    className="chat-content-card flex items-center gap-2.5 px-3 py-2"
                  >
...
function ChatSystemMessage({ message, className }: ChatSystemMessageProps) {
  return (
    <div
      className={cn("chat-system-message my-4 flex justify-center", className)}
    >
    ...
    <div className="relative shrink-0 rounded-lg border border-[var(--chat-border-strong)] bg-[var(--chat-bg-sidebar)]">
      {isImage && item.preview ? (
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <div className="min-w-0">
            <p className="max-w-[120px] truncate text-[12px] font-medium text-[var(--chat-text-primary)]">
              {item.file.name}
            </p>
      ...
      {item.progress !== undefined && item.progress < 100 && (
        <div className="absolute bottom-0 left-0 h-[3px] w-full bg-[var(--chat-border)]">
          <div
            className="h-full bg-[var(--chat-accent)] transition-all"
      ...
      {isDragging && (
        <div className="chat-drop-overlay">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[14px] font-medium text-[var(--chat-accent)]">
            ...
            <div className="relative">
              <button
                ...
                className={cn(
                  ...
                  showAttachMenu &&
                    "rotate-45 bg-[var(--chat-accent-soft)] text-[var(--chat-accent)]",
                )}
