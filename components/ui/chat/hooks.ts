...
export function useAutoScroll(
  ...
  // A new latest message should always be visible in an open conversation.
  // Loading older pages only changes the first message, so it keeps the reader's
  // current position instead of pulling them back to the bottom.
  useEffect(() => {
    const latestMessageId = messages.at(-1)?.id;
    const isNewLatestMessage =
      latestMessageId !== undefined &&
      latestMessageId !== previousLastMessageId.current;
    previousLastMessageId.current = latestMessageId;

    if (isNewLatestMessage) scrollToBottom("smooth");
