...
function ChatHeader({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-10 flex items-center gap-3 border-b border-[var(--chat-border)] bg-[var(--chat-bg-header)] px-4 py-3 backdrop-blur-[20px] backdrop-saturate-[180%]",
        className,
      )}
    >
      {onBack && (
        <button
          onClick={onBack}
          className="mr-1 text-[var(--chat-text-secondary)] hover:text-[var(--chat-text-primary)]"
        >
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between">
          <span className="truncate text-[15px] font-semibold text-[var(--chat-text-primary)]">
            {convo.title}
          </span>
        ...
        <div className="flex items-center justify-between">
          <span className="truncate text-[13px] text-[var(--chat-text-secondary)]">
            {convo.lastMessage}
          </span>
          ...
          {activeConvo ? (
            <>
              <ChatHeader
                ...
                subtitle={
                  subtitle || (activeConvo.isGroup ? "Group" : undefined)
                }
                avatar={
                  <>
            ...
            <div className="flex flex-1 items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[15px] font-medium text-[var(--chat-text-secondary)]">
...
function InlineChat({
  ...
  return (
    <ChatProvider currentUser={currentUser} theme={theme}>
      <div
        className={cn(
          "flex flex-col overflow-hidden rounded-xl border border-[var(--chat-border-strong)] bg-[var(--chat-bg-main)]",
          className,
        )}
        style={{ maxHeight }}
      >
...
function ChatBoard({
  ...
  return (
    <ChatProvider currentUser={currentUser} theme={theme}>
      <div
        className={cn(
          "flex h-full flex-col bg-[var(--chat-bg-main)]",
          className,
        )}
      >
        {activeTopic ? (
          ...
          <>
            <ChatHeader
              title="Discussions"
              actions={
                <button className="flex size-8 items-center justify-center rounded-lg bg-[var(--chat-accent)] text-white">
            ...
            <div className="flex-1 overflow-y-auto">
              {topics.map((t) => (
                ...
                >
                  {t.isPinned && (
                    <Pin className="mt-0.5 size-3.5 shrink-0 text-[var(--chat-orange)]" />
                  )}
                  <div className="min-w-0 flex-1">
                    ...
                    {t.tags && t.tags.length > 0 && (
                      <div className="mt-1 flex gap-1">
                        {t.tags.map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full bg-[var(--chat-accent-soft)] px-2 py-0.5 text-[11px] font-medium text-[var(--chat-accent)]"
                          >
                            {tag}
                          </span>
                        ))}
    ...
    <div className="flex gap-1">
      {tabs.map((t) => (
        <button
          ...
          style={{
            background:
              value === t.key ? "var(--chat-accent-soft)" : "transparent",
            color:
              value === t.key
                ? "var(--chat-accent)"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-1">
          <span className="text-[10px] font-mono text-[var(--chat-text-tertiary)]">
            {ticket.id}
          </span>
          <span className="shrink-0 text-[9px] text-[var(--chat-text-tertiary)]">
            {ticket.createdAt}
          </span>
        </div>
