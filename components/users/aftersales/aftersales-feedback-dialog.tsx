...
const CustomerEmployeeAfterSales = ({
    ...
    const [selectedDetail, setSelectedDetail] = useState<WithFeedbackProps | MyCustomerResolved | null>(null);
    ...
    const columns: ColumnDef<WithFeedbackProps | MyCustomerResolved>[] = [
        {
            ...
            cell: ({ row }) => <Link onClick={(e) => {
                e.stopPropagation()
            }} className="hover:underline" target="_blank" href={`/${base_route}/${row.original.member ? "member" : "customer"}/${row.original.id}`}><div className="ml-2">{row.getValue("owner")}</div></Link>,
        },
        {
            accessorKey: "name",
            filterFn: "includesString",
            ...
            cell: ({ row }) => <Link onClick={(e) => {
                e.stopPropagation()
            }} className="hover:underline" target="_blank" href={`/${base_route}/${row.original.member ? "member" : "customer"}/${row.original.id}`}><div>{row.getValue("name")}</div></Link>,
        },
...
function FeedbackDetailDialog({
    ...
    const feedbackStatus = item && "feedback_status" in item ? item.feedback_status : undefined;
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-full sm:max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>Complete selected customer feedback record.</DialogDescription>
                </DialogHeader>
                <ScrollArea className="max-h-[calc(100dvh-160px)]">
                    {item ? (
                        <div className="space-y-4">
                            <div className={`rounded-2xl border p-4 ${isSatisfactory ? "border-emerald-200 bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/30" : "border-rose-200 bg-rose-50 dark:border-rose-900/60 dark:bg-rose-950/30"}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`flex size-12 items-center justify-center rounded-full ${isSatisfactory ? "bg-emerald-600 text-white" : "bg-rose-600 text-white"}`}>
                                        {isSatisfactory ? <Smile className="size-6" /> : <Frown className="size-6" />}
                                    </div>
                                    <div className="min-w-0">
                                        <p className={`text-sm font-semibold ${isSatisfactory ? "text-emerald-900 dark:text-emerald-200" : "text-rose-900 dark:text-rose-200"}`}>
                                            {isSatisfactory ? "Satisfactory Feedback" : "Unsatisfactory Feedback"}
                                        </p>
                                        <p className={`text-xs ${isSatisfactory ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}>
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                                {detailRows.map(([key, value]) => (
                                    ...
                                    <div key={key} className="rounded-xl border border-border bg-muted/40 p-3">
                                        <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{formatDetailLabel(key)}</p>
                                        <p className="mt-1 break-words text-sm font-medium text-foreground">{formatDetailValue(value, key)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
...
function formatDetailValue(value: unknown, key: string) {
    if (key === "created_at" || key === 'feedback_date') return value ? moment(value).format("YYYY-MM-DD") : "-"
