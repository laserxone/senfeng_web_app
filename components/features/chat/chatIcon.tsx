                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <p className="shrink-0 text-[10px] text-muted-foreground">
                      {item?.conversation?.last_updated
                        ? moment(new Date(item.conversation.last_updated)).format("MMM D, h:mm A")
                        : ""}
                    </p>
                  </div>
...
const RenderReadCount = ({ unread }: { unread: number }) => {
  if (unread && Number(unread) > 0)
    return (
      <div className="grid min-w-5 place-items-center rounded-full bg-primary px-1.5 py-0.5 text-[10px] font-semibold leading-none text-primary-foreground">
        {unread > 99 ? "99+" : unread}
      </div>
    );
};
