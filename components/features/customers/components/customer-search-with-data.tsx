...
export function CustomerSearchWithData({
  value,
  onReturn,
}: {
  value: MyCustomer | null
  onReturn: (val: MyCustomer) => void
}) {
  ...
  React.useEffect(() => {
    async function fetchData() {
      const response: { data: MyCustomer[] } = await axios.get(
        `/${userID}/mycustomer`
      )
      if (response.data.length > 0) {
        ...
        const finalData = apiData.map((item) => {
          ...
          return {
            ...item,
            search: [item.name, item.owner, item.location, ...numbers]
              .filter(Boolean)
              .join(" ")
              .trim(),
          }
