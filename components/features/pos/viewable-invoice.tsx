...
export default function ViewableInvoice({
  ...
  return (
    <section className="mb-5 w-full min-w-0 rounded-md border border-[#BBD9F4] bg-[#F1F7FF] p-2 shadow-sm sm:p-3">
      <div
        ref={pdfRef}
        className="min-w-0 overflow-hidden rounded-md border-2 border-[#0072BC] bg-white"
      >
        <div className="flex flex-col gap-3 border-b border-[#D6E9FA] bg-[#F1F7FF] p-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <img
              src="/logo.png"
              alt="Senfeng Pakistan"
              className="h-10 w-44 object-contain sm:h-12 sm:w-56"
            />
        ...
        <div className="grid gap-3 p-3 lg:grid-cols-[1fr_260px]">
          <div className="rounded-md border border-[#D6E9FA] bg-[#F8FBFF] p-3">
            <p className="mb-2 text-xs font-bold tracking-wide text-[#0072BC] uppercase">
              Bill To
            </p>
            <div className="grid gap-2 sm:grid-cols-2">
              {customerFields.map((field) => (
                <div
                  key={field.label}
                  className={field.label === "Address" ? "sm:col-span-2" : ""}
                >
          ...
          <div className="rounded-md border border-[#D6E9FA] bg-[#F8FBFF] p-3 text-xs">
            <p className="text-base font-bold text-[#0072BC]">
              SENFENG PAKISTAN
            </p>
            <p className="mt-2 text-[#7F7F7F]">
              Street# 2, Sharif Garden Daroghawala, Lahore, Punjab 54000,
              Pakistan
            </p>
            <p className="mt-2 break-words text-[#7F7F7F]">
            ...
            <table className="w-full min-w-[620px] border-collapse text-xs">
              <thead>
                <tr className="bg-[#0072BC] text-white">
                  {[
                    ...
                    <th
                      key={header}
                      className="border border-[#D1D5DB] px-2 py-2 text-left font-semibold"
                    >
                      {header}
                    </th>
                  ))}
              ...
              <tbody>
                {invoiceItems.map((item, i) => (
                  <tr
                    key={i}
                    className={i % 2 === 0 ? "bg-[#f1f1f1]" : "bg-white"}
                  >
                    <td className="border border-[#D1D5DB] px-2 py-1.5 text-black">
                      {i + 1}
                    </td>
                ...
                {[...Array(blankRows)].map((_, i) => (
                  <tr key={i} className="h-7 bg-white text-xs">
                    <td className="border border-[#D1D5DB] px-2 text-black">
          ...
          <div className="flex w-full justify-end">
            <div className="w-full max-w-sm overflow-hidden rounded-md border border-[#0072BC] text-xs sm:text-sm">
              <div className="grid grid-cols-[1fr_1.4fr] bg-[#0072BC] text-white">
                <div className="px-3 py-2 leading-tight font-semibold">
                  Discount
                </div>

                <div className="border-l border-white/70 px-3 py-2 text-right leading-tight font-semibold">
              ...
              <div className="grid grid-cols-[1fr_1.4fr] border-t border-white/70 bg-[#005f9e] text-white">
                <div className="px-3 py-3 leading-tight font-bold">
                  Total Amount
                </div>

                <div className="border-l border-white/70 px-3 py-3 text-right leading-tight font-bold">
              ...
              <table className="w-full min-w-[360px] border-collapse text-xs">
                <tbody>
                  {bankRows.map(([label, value], index) => (
                    <tr
                      key={label}
                      className={index % 2 ? "bg-[#FFE4E1]" : "bg-white"}
                    >
                      <td className="w-32 border border-[#D1D5DB] px-2 py-2 text-black">
                        {label}
                      </td>
                      <td className="border border-[#D1D5DB] px-2 py-2 font-bold text-[#0072BC]">
                        {value}
                      </td>
                    </tr>
                  ))}
        ...
        {warranty && (
          <div className="mx-3 mb-3 rounded-md border border-[#D1D5DB] bg-white p-3 text-xs leading-relaxed">
            <p className="font-semibold text-red-600">
              ...
              fluctuation, inexpert repair, improper transportation, unsuitable
              storage or use under harsh environment or conditions at
              Buyer&apos;s end.
            </p>
            <p className="mt-2 text-center text-sm font-bold text-orange-500">
            ...
            <p className="mt-1 text-black">
              ...
              repair and maintenance can only be done by Raycus/MAX China within
              warranty time. Customer will send and receive the equipment to
