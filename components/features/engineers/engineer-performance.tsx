    ...
    <div className="flex flex-1 flex-col space-y-4 pb-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm sm:p-5">
        <Heading
          ...
          description="Review engineering team performance"
        />
        <Button onClick={() => setFilterVisible(true)} variant="outline">
          <Filter />
          {startDate && endDate && (
            <Label>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              ...
              variant="primary"
              trend={{
                value: data?.overview?.total_assigned_change?.value || 0,
                isPositive:
                  data?.overview?.total_assigned_change?.positive || false,
              }}
            />
            <StatCard
              title="Completed"
              ...
              variant="success"
              trend={{
                value: data?.overview?.total_completed_change?.value || 0,
                isPositive:
                  data?.overview?.total_completed_change?.positive || false,
              }}
            />
            <StatCard
              title="Pending"
              ...
              variant="warning"
              trend={{
                value: data?.overview?.total_pending_change?.value || 0,
                isPositive:
                  data?.overview?.total_pending_change?.positive || false,
              }}
            />
            <StatCard
              ...
              variant="primary"
              trend={{
                value: data?.overview?.completion_rate_change?.value || 0,
                isPositive:
                  data?.overview?.completion_rate_change?.positive || false,
              }}
            />
          </div>
