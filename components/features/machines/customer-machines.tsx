...
export function CustomerMachines({
  ...
  onReturn,
  customer_id
}: { value: number | null, onReturn: (val : number)=> void,  customer_id: string | number | null }) {
  ...
  React.useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const response = await axios.get(`/${userID}/customer-machines?customer_id=${customer_id}`)
        ...
        const apiData = data
          .map((item) => {
            ...
            return {
              ...item,
               label: order_no.trim() ? order_no : `${item.serial_no ?? ""} ${item.power ?? ""} ${item.source ?? ""}`
            };
          })


        const finalData = apiData.map((item, index) => {
          return { ...item, value: item.id, label: `${index + 1}- ${item.label}` };
        });
        ...
        <Command>
          <div className="p-1 pb-0">
            <InputGroup className="h-8! rounded-lg! border-input/30 bg-input/30 shadow-none! *:data-[slot=input-group-addon]:pl-2!">
              <InputGroupInput id="inline-start-input" placeholder="Search machine..." value={search}
