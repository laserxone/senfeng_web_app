...
function generateAttendanceData(rawData: AttendanceTableRow[], start: string, end: string) {
    ...
    const uniqueUsers = Array.from(new Set(rawData.map((item) => item.user_email).filter(Boolean)));
    ...
    const finalData = uniqueUsers.flatMap((user) =>
        datesInMonth.map((date) => {
            const match = rawData.find((item) => item.user_email === user && moment(item.date).format("YYYY-MM-DD") === date);
            return {
                ...match,
                date,
                user_email: user,
    ...
    return finalData.filter((item) => item.date <= today).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

export {formatCurrency, formatDate, isCompleted, isResolved, percent, generateAttendanceData}