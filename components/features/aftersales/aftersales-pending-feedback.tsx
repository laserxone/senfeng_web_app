        ...
        <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-2 rounded-xl border border-border bg-card p-2 text-card-foreground shadow-sm sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-sm font-semibold text-foreground">Pending Feedback</h2>
                        ...
                        >
                            <div className="flex min-w-0 items-start gap-3 xl:border-r xl:border-border xl:pr-4">
                                ...
                                <div className="min-w-0 flex-1">
                                    <Link className="hover:underline" target="_blank" href={`/${base_route}/member/${item.id}`}>
                            ...
                            <div className="min-w-0 space-y-3">
                                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                    ...
                                    <div className="flex shrink-0 flex-wrap items-center gap-2 lg:justify-end">
                                        <InfoTag icon={<CalendarDays className="size-3.5" />} value={formatDate(item.previous_feedback_date)} />
                                        <InfoTag icon={<MapPin className="size-3.5" />} value={formatValue(item.location)} />
                                        <InfoTag
                                ...
                                <FeedbackForm user_id={user_id} onRefresh={onRefresh} customer_id={item.id} />
                            </div>
                        </div>
                    ))
                ) : (
...
const Rating = ({ rating }: { rating: number }) => {
    ...
    return (
        <div className="mt-1 flex items-center gap-1">
            ...
            <span className="text-[11px] font-medium text-muted-foreground">({displayRating.toFixed(1)})</span>
        </div>
    );
};
...
const InfoTag = ({
    ...
    return (
        <span className={`inline-flex max-w-full items-center gap-1.5 rounded-md border px-2 py-1 text-[11px] font-semibold ${toneClass}`}>
            {icon}
...
function objectValuesToSearchText(value: unknown): string {
    ...
    if (typeof value === "object") return Object.values(value).map(objectValuesToSearchText).join(" ");
...
function getStatusTone(value: unknown) {
    ...
    if (status.includes("satisfactory") && !status.includes("unsatisfactory")) return "success";
