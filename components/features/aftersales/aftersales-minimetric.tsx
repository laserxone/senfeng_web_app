...
export default function MiniMetric({ label, value, icon, tone }: { label: string; value: ReactNode; icon: ReactElement; tone: MetricTone }) {
    return (
        <div className="min-w-0 rounded-xl px-2 py-1.5">
            <div className="flex items-center gap-2">
                <span className={`flex size-6 shrink-0 items-center justify-center rounded-lg ring-1 ${toneClasses[tone]}`}>
                    {icon}
                </span>
                <div className="min-w-0">
                    <p className="truncate text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{label}</p>
                    <p className="truncate text-base font-semibold tracking-tight text-foreground">{value}</p>
                </div>
            </div>
        </div>
    );
