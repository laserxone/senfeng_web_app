        ...
        <div className="flex w-full flex-col gap-3">
          ...
          {userID && (
            <AfterSalesDashboard
              ...
              data={{
                withFeedback:
                  feedbacks?.withFeedback.data.map((item) => ({
                    ...item,
                    number: item.number.join(", "),
                  })) ?? [],
                withoutFeedback:
                  feedbacks?.withoutFeedback.data.map((item) => ({
                    ...item,
                    number: item.number.join(", "),
                  })) ?? [],
              }}
    ...
    <>
      <DashboardStatsCard
        ...
        metrics={[
          ...
          {
            label: "Satisfied",
            value: analytics.satisfied,
            icon: <ThumbsUp className={iconSize} />,
            tone: "green",
          },
          {
            label: "Unsatisfied",
            value: analytics.unsatisfied,
      ...
      <FeedbackDialog
        data={{
          withFeedback:
            data?.withFeedback.data.map((item) => ({
              ...item,
              number: item.number.join(", "),
            })) ?? [],
          withoutFeedback:
            data?.withoutFeedback.data.map((item) => ({
              ...item,
              number: item.number.join(", "),
            })) ?? [],
        }}
      ...
      <DashboardStatsCard
        ...
        metrics={[
          ...
          },
          {
            ...
            icon: <Clock3 className={iconSize} />,
            tone: "amber",
          },
          {
            label: "Resolved",
  ...
  return (
    <DashboardStatsCard
      ...
      metrics={[
        {
          label: "Total Sales",
          value: <CurrencyFormatter amount={totalSales} showPKR={false} />,
          icon: <Banknote className={iconSize} />,
          tone: "blue",
        },
        {
          label: "Pending",
          value: <CurrencyFormatter amount={pending} showPKR={false} />,
          icon: <Clock3 className={iconSize} />,
          tone: "amber",
        },
        {
          label: "Completed",
          value: <CurrencyFormatter amount={completed} showPKR={false} />,
          icon: <CheckCircle2 className={iconSize} />,
          tone: "green",
        },
        {
          ...
          icon: <CircleDollarSign className={iconSize} />,
          tone: "violet",
        },
      ]}
    />
      ...
      <DashboardStatsCard
        ...
        metrics={[
          ...
          },
          {
            ...
            icon: <UserMinus className={iconSize} />,
            tone: "red",
          },
          {
            label: "Late",
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[88vh] overflow-hidden sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Present, absent, late, and attendance percentage by team member.
            </DialogDescription>
          </DialogHeader>
          <AttendanceTable
            users={analytics.users.map((item) => ({
              ...item,
              rate: percent(item.present + item.late, item.total),
            }))}
          />
        </DialogContent>
      </Dialog>
    ...
    <section className="rounded-2xl border border-border bg-card p-3 text-card-foreground shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-2">
        <MiniMetric
          label="Requests"
          value={data.length}
          icon={<ReceiptText className={iconSize} />}
          tone="blue"
        />
        <MiniMetric
          label="Pending"
