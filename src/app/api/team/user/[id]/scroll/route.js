...
export async function GET(req, { params }) {
    try {
        ...
        const condition = await pool.query(`SELECT designation, limited_access FROM users WHERE id = $1`, [id])
        ...
        if (user.limited_access) {
            if (user.designation === 'Customer Relationship Manager' || user.designation === 'Customer Relationship Manager (After Sales)' || user.designation === 'Social Media Manager') {
                result = await pool.query(`SELECT id, name, owner, location, member FROM customer WHERE lead = $1`, [id])
            } else if (user.designation === 'Sales' || user.designation === 'Engineer') {
                result = await pool.query(`SELECT id, name, owner, location, member FROM customer WHERE ownership = $1`, [id])
            }
        } else {
