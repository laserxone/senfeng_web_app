...
`;
    ...
    FROM 
        payment p
    JOIN 
        sale m ON p.machine_id = m.id
    JOIN 
        users u ON m.sell_by = u.id
    WHERE 
...
GROUP BY 
  ...
  END;
        ...
        LEFT JOIN users u ON f.user_id = u.id AND u.office = '${city}'
...
FROM task
JOIN users ON task.assigned_to = users.id AND users.office = '${city}'
