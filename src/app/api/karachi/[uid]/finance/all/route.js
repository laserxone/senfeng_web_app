...
export async function GET(req) {
    ...
    WHERE u.office = $1 AND s.contract_date >= '2025-01-01'
  `;

        const { rows: sales } = await pool.query(query, [office]);
    ...
    `;
        ...
        const normalizedSales = sales.map((sale) => {
            const salePayments = payments.filter((p) => p.machine_id === sale.machine_id);
            const totalPaymentReceived = salePayments.reduce(
                (sum, p) => sum + Number(p.amount),
                0
            );
            const speedMoneyDeduction = sale.speed_money ? sale.speed_money_amount ? Number(sale.speed_money_amount) : 0 : 0;
            const balance = Number(sale.total_generated) - totalPaymentReceived - speedMoneyDeduction;
