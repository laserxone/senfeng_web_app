...
export async function GET(req) {
  ...
  try {
    if (availablemachine) {
      ...
      const result = await pool.query(
        "SELECT * FROM inventory_karachi ORDER BY id ASC",
      );

      const reminders = await pool.query(
        "SELECT * FROM savedinvoices_karachi WHERE payment=false",
      );
  ...
  try {
    ...
    if (selecteduser?.id) {
      ...
      const lastIdResult = await pool.query(
        "SELECT MAX(id) AS last_id FROM savedinvoices_karachi",
      );
    ...
    const result = await pool.query(
      "SELECT id FROM poscustomer_karachi WHERE phone = $1 LIMIT 1",
      [phone],
    );
