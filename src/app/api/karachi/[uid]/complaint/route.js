...
export async function GET(req, { params }) {
  ...
  try {
    ...
    //     WHERE c.customer_id IS NOT NULL AND owner_user.office = 'karachi'
    //   `;
    ...
    // Not superadmin
    const userQuery = await pool.query(`SELECT id, designation, complaint_assigned FROM users WHERE id = $1`, [uid]);
    ...
    if (user.complaint_assigned || isAdmin) {
      ...
      `;
        ...
        return NextResponse.json({ message: "No data to display" }, { status: 404 });
      }
    }
    const result = await pool.query(query, queryParams);
    return NextResponse.json(result.rows, { status: 200 });
