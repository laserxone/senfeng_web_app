...
export default function Page() {
    ...
    const [commulative, setCommulative] = useState({ total: 0, pending: 0, received: 0 })
    ...
    async function fetchCommulative() {
        setCommloading(true)
        axios.get(`/${userID}/finance/all`).then((response) => {
    ...
    const columns = [
...

        {
            ...
            cell: ({ row }) => <div>{row.getValue("machine_price")}</div>,
        },
