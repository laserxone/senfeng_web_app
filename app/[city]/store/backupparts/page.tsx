        ...
        },
        {
            ...
            cell: ({ row }) => (
                <DateTimeCell
                    ...
                    success={Boolean(row.original.backup_application_detail?.actual_return_date)}
                />
            ),
        },
        {
...
function StatusBadge({ part }: { part: BackupPart }) {
    if (isPartOverdue(part)) {
        return (
            <span className="inline-flex whitespace-nowrap rounded-md bg-red-50 px-2.5 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-200">
                Overdue
            </span>
        );
    }
    ...
    if (getPartStatus(part) === "in_stock") {
        return (
            <span className="inline-flex whitespace-nowrap rounded-md bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 ring-1 ring-inset ring-emerald-200">
                In Stock
            </span>
        );
    }
    ...
    return (
        <span className="inline-flex whitespace-nowrap rounded-md bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-700 ring-1 ring-inset ring-orange-200">
            Given to Customer
        </span>
    );
}
...
function DateTimeCell({
    ...
    return (
        <span
            className={`text-xs font-medium leading-5 ${danger
                ? "text-red-600"
                : success
                    ? "text-emerald-600"
                    : "text-slate-700"
