...
export default function BackendPage() {
  ...
  return (
    <div className="flex flex-1 flex-col space-y-4">
      ...
      <div
        className={`relative flex min-h-[calc(100dvh-290px)] flex-1 flex-col ${isMobile && "min-h-[500px]"}`}
      >
        <div className="custom-scrollbar absolute top-0 right-0 bottom-0 left-0 flex overflow-auto rounded-md border md:overflow-auto">
          {selected ? (
              ...
              <TableBody>
                {paginatedRows.map((row) => (
                  <TableRow key={String(row.id) || JSON.stringify(row)}>
                    {columns.map((col) => (
                      <TableCell key={col.name} className="w-[180px] align-top">
      ...
      {selected && (
        <div className="mt-3 flex items-center justify-between">
          <div className="text-sm">
            ...
            - {Math.min(filteredRows.length, page * PAGE_SIZE)} of{" "}
            {filteredRows.length}
          </div>
