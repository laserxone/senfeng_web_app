    ...
    ({ data }: { data: UserMap[] }) => {
      ...
      return (
        <div className="flex flex-1 flex-col gap-4 sm:flex-row">
          <MapProvider>
            ...
            >
              ...
              {selected !== null && (
                ...
                >
                  <div className="rounded-md bg-white text-sm text-gray-800 shadow-md dark:text-black">
          ...
          <ScrollArea className="flex h-[500px] w-full flex-col pr-2 sm:h-[80vh] sm:w-[400px]">
            {data.map((item, index) => {
              const label =
                index === 0
                  ? "Start"
              ...
              return (
                <div
                  key={index}
                  onClick={() => setSelected(index)}
                  className={`mb-2 cursor-pointer rounded-md border px-3 py-2 transition hover:opacity-70 ${
                    theme === "dark"
    ...
    <div className="flex flex-1 flex-col space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-card p-4 shadow-sm sm:p-5">
        <Heading
          panel
          title="Map record"
          description="View user locations record"
        />
        <div>
          {userID && (
