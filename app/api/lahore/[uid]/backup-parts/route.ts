...
export async function GET(req: NextRequest) {
  try {
    ...
    const applicationMap = new Map(
      applicationResult.rows.map((item) => [
        item.backup_inventory_id,
        item,
      ])
    );
    ...
    const data = inventoryResult.rows.map((inventory) => ({
      ...inventory,
      backup_application_detail:
        applicationMap.get(inventory.id) ?? null,
        status : applicationMap.get(inventory.id) ? "given_to_customer" : "in_stock" 
    }));
