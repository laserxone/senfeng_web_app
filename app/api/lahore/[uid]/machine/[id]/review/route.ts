...
export const createMachineReviewHandler = (office: "lahore" | "karachi") =>
  async function POST(
    req: NextRequest,
    ...
    try {
      ...
      if (action === "resubmit") {
        if (Number(machine.sell_by) !== Number(uid)) {
          return NextResponse.json(
            {
              message:
                "Only the employee who added this machine can resubmit it",
            },
            { status: 403 },
          );
        }
      ...
      if (action === "rejected" && !comment?.trim()) {
        return NextResponse.json(
          { message: "A comment is required when rejecting a machine" },
          { status: 400 },
        );
      }
      if (
        ...
        return NextResponse.json(
          { message: "Only an owner of this office can review this machine" },
          { status: 403 },
        );
      }
      if (machine.review_status !== "pending") {
        return NextResponse.json(
          { message: "This machine is not awaiting review" },
          { status: 409 },
        );
      }
      ...
      return NextResponse.json(
        { message: "Unable to update machine review" },
        { status: 500 },
      );
    } finally {
