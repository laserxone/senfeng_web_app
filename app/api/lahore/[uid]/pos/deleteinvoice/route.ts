...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const partsQuery = await client.query(
      `SELECT id, image FROM customer_parts WHERE part_id = $1`,
      [inv_id]
    )
    ...
    await client.query(`DELETE FROM customer_parts WHERE part_id = $1`, [
      inv_id,
    ])

    await client.query(`DELETE FROM savedinvoices WHERE id = $1`, [inv_id])
