...
export const createDeliveryHandlers = (office: "lahore" | "karachi") => {
  async function GET() {
    try {
...
`,
  ...
  async function POST(req: NextRequest) {
    ...
    try {
      ...
      const linkedOrderItems = await client.query(
        `UPDATE order_items
       SET status = $1
       WHERE machine_id = $2
       RETURNING id`,
      ...
      if (linkedOrderItems.rowCount === 0) {
        const selectedSerial = data.order_no_data?.machine_serial;

        if (selectedSerial) {
          const availableOrderItems = await client.query(
            `SELECT id
           FROM order_items
           WHERE machine_serial = $1 AND booked IS FALSE
           FOR UPDATE`,
            [selectedSerial],
          );

          if (availableOrderItems?.rows?.length > 0) {
            await client.query(
              `UPDATE order_items
             SET status = $1, machine_id = $2
             WHERE id = $3`,
              ["Dispatched", data.machine_id, availableOrderItems.rows[0]?.id],
            );
          }
        }
      }
      ...
      VALUES (
        $1,$2,$3, $4
          ...
          [true, data.transportation, data.machine_id, office],
        );
      }
...
export const { GET, POST, PUT, DELETE } = createDeliveryHandlers("lahore");
