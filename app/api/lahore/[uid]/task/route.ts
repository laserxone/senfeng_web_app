...
export async function POST(req: NextRequest) {
    ...
    `;
        ...
        if (assigned_by && assigned_by !== assigned_to) {
            ...
            sendNotificationToOwner(`Task assigned: ${taskName}`,`task?t=${newTask.rows?.[0]?.id}&start=${start}&end=${end}`, "lahore", NOTIFICATION_TYPES.task_assigned.category, NOTIFICATION_TYPES.task_assigned.title )
        }
