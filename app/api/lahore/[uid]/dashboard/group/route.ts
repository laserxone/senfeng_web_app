...
export async function GET(
  ...
  try {
    const [userResult] = await Promise.all([
      pool.query(
        "SELECT id, dp, name, designation, limited_access FROM users WHERE id = $1",
        [id],
      ),
    ]);
    ...
    if (user?.designation === "Social Media Manager") {
      ...
      customers.forEach((customer) => {
        ...
        const nextFollow = latest.next_followup
          ? oldMoment(latest.next_followup)
          : null;
        ...
        // Category: Next Month
          ...
          nextFollow.isBetween(startOfNextMonth, endOfNextMonth, null, "[]")
        ) {
