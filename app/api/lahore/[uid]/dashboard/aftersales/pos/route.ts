...
`;
        ...
        const total_pending = invoices.filter(
            (item) =>
                moment(item.created_at).isSameOrAfter("2025-12-01") ||
                item.payment === false
        ).filter((item) => item.status !== "Paid").reduce((sum, item) => sum + (item.final_amount || 0), 0)

        const total_sales = invoices.reduce((sum, item) => sum + (item.final_amount || 0), 0)
